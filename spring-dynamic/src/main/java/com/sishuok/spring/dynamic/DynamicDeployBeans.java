...
public class DynamicDeployBeans {
	...
	private static Method getPathMatcherMethod = ReflectionUtils.findMethod(RequestMappingHandlerMapping.class,
			"getPathMatcher");
	private static Field handlerMethodsField = ReflectionUtils.findField(RequestMappingHandlerMapping.class,
			"handlerMethods", Map.class);
	private static Field urlMapField = ReflectionUtils.findField(RequestMappingHandlerMapping.class, "urlMap",
			MultiValueMap.class);
	...
	private Map<String, Long> scriptLastModifiedMap = new ConcurrentHashMap<>();// in
																				// millis
			...
			try {
				...
				if (!ObjectUtils.isEmpty(beanNames)) {
					for (String s : beanNames) {
						if (s.contains(beanClass.getName())) {
							beanFactory.removeBeanDefinition(s);
						}
					}
				}
	...
	public void registerController(Class<?> controllerClass) {
		...
		context.setConfigLocation("classpath:spring-mvc-test.xml");
		...
		try {
			loadBeanDefinitions.invoke(context, beanFactory);
	...
	public void removeDefaultBeans(Class<?> beanClass) {
		String[] beanNames = beanFactory.getBeanNamesForType(beanClass);
		if (!ObjectUtils.isEmpty(beanNames)) {
			for (String s : beanNames) {
				if (s.contains(beanClass.getName())) {
	...
	public void removeBeans(Class<?> beanClass) {
		String[] beanNames = beanFactory.getBeanNamesForType(beanClass);
		if (!ObjectUtils.isEmpty(beanNames)) {
			for (String s : beanNames) {
				beanFactory.removeBeanDefinition(s);
	...
	public void registerGroovyController(String scriptLocation) throws IOException {
		...
		Object controller = groovyScriptFactory
				.getScriptedObject(new ResourceScriptSource(ctx.getResource(scriptLocation)));
		...
		if (beanFactory.containsBean(controllerBeanName)) {
			...
			removeInjectCache(controller); // 移除注入缓存 否则Caused by:
	...
	private void removeInjectCache(Object controller) {
		...
		AutowiredAnnotationBeanPostProcessor autowiredAnnotationBeanPostProcessor = ctx
